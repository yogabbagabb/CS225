...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    auto pos = new_right->elements.begin();
    new_right->elements.insert(pos, mid_elem_itr + 1, child->elements.end());
    auto possy = new_right->children.begin();
    new_right->children.insert(possy, mid_child_itr, child->children.end());
