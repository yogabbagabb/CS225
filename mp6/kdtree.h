...
class KDTree
{
  ...
  private:
    ...
    void order(int left, int right, std::vector<Point<Dim>> & points, int count);

    void quickselect(int left, int right, int half, std::vector<Point<Dim>> & points, int count);

    int partition(int left, int right, int pivot, std::vector<Point<Dim>> & points, int count);

    Point<Dim> descend (int left, int right, int count, const Point<Dim> & query) const;

    void swap(Point<Dim> & a, Point<Dim> & b);

    int radiusSquared(const Point<Dim> &a, const Point<Dim> &b) const;
    int max(int a, int b) const;
    int absDSquared(int a, int b) const;
