...
// Why doesn't *original(xi, yi).red = 0 work?
// what is original -- a pointer or an instance?
// Why can't I print out pixel.red using cout

		size_t limit = 255;
		RGBAPixel pixel = *original(xi, yi);		
		pixel.red = (pixel.red + amount <= 255)? (pixel.red + amount) : limit;
		pixel.green = (pixel.green + amount <= 255)? (pixel.green + amount) : 											limit;
		pixel.blue = (pixel.blue + amount <= 255)? (pixel.blue + amount) : limit;

		*original(xi,yi) = pixel;
