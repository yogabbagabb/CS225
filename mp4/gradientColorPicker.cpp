...
gradientColorPicker::gradientColorPicker(RGBAPixel fadeColor1,
                                         ...
                                         int centerX, int centerY) : fadeOne(fadeColor1),
                                         fadeTwo(fadeColor2), cx(centerX),
                                         cy(centerY), r(radius)
{
    /**
...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
    ...
    if (d <= r)
    {
      color.red = getScaledComponent((int)fadeOne.red, (int)fadeTwo.red, d);
      color.green = getScaledComponent((int)fadeOne.green, (int)fadeTwo.green, d);
      color.blue = getScaledComponent((int)fadeOne.blue, (int)fadeTwo.blue, d);
    }
    else
    {
      color = fadeTwo;
...
uint8_t gradientColorPicker::getScaledComponent(int c1, int c2, int d)
{
    return c1 + floor(c2*d/r) - floor(c1*d/r);
}
